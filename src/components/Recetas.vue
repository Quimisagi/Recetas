...
<div>
    ...
    <br>
    <form>
      <input class="buscadorID" type="text" placeholder="Ingrese el nombre de la receta" v-model="buscador"> 
      <button type="button" class="botonAzul" @click="buscarRecetaNombre(buscador)">Buscar</button>
      <button type="button" class="botonAzul" @click="fetchRecetas">Lista</button> <br>
    </form>
    <div>   
    <ul>
      <li class="listaRecetas"
       v-for="(receta, index) in recetas" :key='index'> 
        ...
        <div class="botones">
      <button type="button" class="botonActualizar" @click="openModalActualizar(receta.id, receta.nombre, receta.ingredientes, receta.instrucciones)"></button>
      <button type="button" class="botonEliminar" @click="openModalConfirmacion(receta.id)"></button> 
        </div>   
      <br>
      Ingredientes:
      <ul>
    ...
    <button type="button" class="buttonAgregar" @click="openModalAgregar"></button> 
...
<div id="modalAgregar" v-if="showModalAgregar">
  <div class="modal-backdrop">
    ...
    <form>
        ...
        <button type="button" class="agregarIngrediente" @click="agregarIngrediente"></button>
        <br>
        <ul>
      <li class="ingredientes" v-for="(data, index) in Receta.ingredientes" :key='index'>{{index}}. {{ data }}
        <button type="button" class="eliminarIngrediente" @click="eliminarIngrediente(index)"></button> 
      </li>
        </ul>
        <br>
        ...
        <br>
          ...
          <button type="button" class="botonRojo" @click="modalClose">Cancelar</button>
    </form> 
  </div>
...
<div id="modalConfirmacion" v-if="showModalConfirmacion">
  <div class="modal-backdrop">
    ...
    <p>Desea eliminar esta receta?</p>
          ...
          <button type="button" class="botonAzul2" @click="eliminarReceta(id)">Sí</button>
          <button type="button" class="botonRojo2" @click="showModalConfirmacion = false">No</button>
...
export default {
  ...
  methods: {
    ...
    buscarRecetaNombre: function(id) {
      this.recetas = [];
      axios.get("http://localhost:8000/api/listaRecetas/" + id).then(
        response => {
          this.recetas = this.recetas.concat(response.data);
